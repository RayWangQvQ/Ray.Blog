...
namespace Ray.Blog.Blazor.Pages.Admin
{
    public partial class AdminPosts
    {
        ...
        async Task OnImageUploadEnded(FileEndedEventArgs e)
        {
            ...
            var imgUri = await BlobAppService.UploadPostPics(new RemoteStreamContent(stream, e.File.Name));

            var hostBaseAddress = Configuration["RemoteServices:Default:BaseUrl"];
            e.File.UploadUrl = $"{hostBaseAddress}{imgUri}";// We need to report back to Markdown that upload is done. We do this by setting the UploadUrl.
